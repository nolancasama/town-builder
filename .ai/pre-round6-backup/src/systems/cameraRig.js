...
export function createCameraRig(camera, domElement) {
  ...
  const targetBeforeMove = new THREE.Vector3();
  ...
  window.addEventListener('keydown', (event) => {
    if (!panKeys.has(event.code) || isTextEntry(event.target)) return;
    pressedKeys.add(event.code);
    event.preventDefault();
  });
  window.addEventListener('keyup', (event) => {
    if (panKeys.has(event.code)) pressedKeys.delete(event.code);
  });
  ...
  function updateKeyboardPan(dt) {
    if (cinematic || !controls.enabled || pressedKeys.size === 0) return;
    const forwardAmount =
    ...
    keyboardForward.subVectors(controls.target, camera.position).setY(0);
    if (keyboardForward.lengthSq() < 0.0001) return;
    ...
    keyboardMove
      ...
      .addScaledVector(keyboardRight, rightAmount)
      .normalize()
      .multiplyScalar(KEYBOARD_PAN_SPEED * dt);

    // Translate the target and camera together, then use the same radial clamp
    // as pointer panning. Adding only the accepted movement avoids edge drift.
    targetBeforeMove.copy(controls.target);
    controls.target.add(keyboardMove);
    clampTarget();
    keyboardMove.subVectors(controls.target, targetBeforeMove);
    camera.position.add(keyboardMove);
  }
