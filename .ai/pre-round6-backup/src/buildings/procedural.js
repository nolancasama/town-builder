...
import { WORLD, LANDMARK_LOTS, ROAD_SEGMENTS, ROAD_WIDTH } from '../config/town.js';
...
export function buildStation({ size, sign, rng, lot }) {
  ...
  const stationWorldZ = lot?.pos?.[1] ?? 15;
  const TRACK_START = -WORLD.size / 2 - stationWorldZ;
  const TRACK_END = WORLD.size / 2 - stationWorldZ;
  // The railway is one straight, level line. Its eastern corridor misses the
  // town's tall landmark sites; the two pads whose outer edges it clips reject
  // the handful of rooflines taller than the deck (see LANDMARK_PLOTS).
  const DECK_Y = 11.2;
  const TRACK_Z = -1.8;
  const DECK_WIDTH = 5.2;
  ...
  const pierClearOfLotsAndRoads = (lx) => {
    // buildStation is tied to lot-station; convert a local pier centre to world
    // coordinates without making the support layout part of the world system.
    const worldX = (lot?.pos?.[0] ?? 44) - PIER_Z;
    const worldZ = stationWorldZ + lx;
    for (const candidate of LANDMARK_LOTS) {
      if (candidate.id === 'lot-station') continue;
