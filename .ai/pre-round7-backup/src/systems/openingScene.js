 ...
 * one character and the existing cinematic/subtitle systems, then removes the
 * character before any buildable lot can be claimed.
 */
export function createOpeningScene({ scene, rig, hud, rng, lot, skip = false }) {
  ...
  async function play(playerName) {
    ...
    // The local has done his one job. Removing him avoids a later landmark
    // intersecting the buildable field where he stood.
    active = false;
    root.removeFromParent();
  }
