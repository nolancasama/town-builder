...
import {
  WORLD, LANDMARK_LOTS, LOT_SIDEWALK_WIDTH, RAILWAY, railwayWorldX,
} from '../config/town.js';
...
export function buildWorld(scene, rng, onProgress = () => {}) {
  ...
  return {
    ...
    ground,
    dressings,
    /** Add the perimeter walk only when construction has claimed this lot. */
    addLotSidewalk(lot) {
      if (lotSidewalks.has(lot.id)) return lotSidewalks.get(lot.id);
      const sidewalk = createLotSidewalk(scene, lot, LOT_SIDEWALK_WIDTH);
