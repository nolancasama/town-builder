...
import { LANDMARK_LOTS, sizeClassFor } from '../config/town.js';
...
function lotIsFree(lot, takenLotIds) {
  return !takenLotIds.has(lot.id);
}

function lotAllowsLandmark(lot, type, def) {
  return (!lot.reservedFor || lot.reservedFor === type)
    && sizeClassFor(lot.plotClass).rank >= sizeClassFor(def.sizeClass).rank
...
export function lotsFor(type, takenLotIds) {
  ...
  return LANDMARK_LOTS.filter(
    (lot) =>
      ...
      lotAllowsLandmark(lot, type, def)
  );
}
...
export function selectLot(type, takenLotIds) {
  ...
  if (def.lot) {
    ...
    if (preferred && lotIsFree(preferred, takenLotIds) && lotAllowsLandmark(preferred, type, def)) return preferred;
  }
  const candidates = lotsFor(type, takenLotIds);
  if (!candidates.length) return null;
  return candidates.sort(
    (a, b) => sizeClassFor(a.plotClass).rank - sizeClassFor(b.plotClass).rank
  )[0];
}
