...
/** Whose house appears in the building pool ("Ken's house" by default). */
export const DEFAULT_HOUSE_OWNER = 'Ken';
export const HOUSE_OWNER_MAX_LENGTH = 20;
export let HOUSE_OWNER = DEFAULT_HOUSE_OWNER;

/**
 * Set at the welcome screen before the world or any building choices are made.
 * Keeping the normalisation here also gives teacher/dev callers the same safe
 * default and length limit as the visible name form.
 */
export function setHouseOwner(value) {
  const name = String(value || '')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, HOUSE_OWNER_MAX_LENGTH)
    .trim();
  HOUSE_OWNER = name || DEFAULT_HOUSE_OWNER;
  return HOUSE_OWNER;
}
