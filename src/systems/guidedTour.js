...
export function createGuidedTour({
  ...
  function presentationPoint(holder) {
    ...
    return new THREE.Vector3(holder.position.x, 0, holder.position.z)
      .add(facing.multiplyScalar(span * 0.5 + 5));
  }
  ...
  async function presentStop(stop, index, total) {
    ...
    if (pedestrians) {
      ...
      pedestrians.addAttraction(entrance[0], entrance[1]);
    }
    await wait(0.4);
  ...
  function canSee(from, to) {
    ...
    const blockers = scene.children.filter(
      (child) => child.visible && child !== root && child.name !== 'clouds' && child.name !== 'birds'
    );
  ...
  function frameStop(holder, groupAt) {
    ...
    for (const lift of [0, 4, 9, 15, 22]) {
      for (const sideMul of [0.45, -0.45, 0.85, -0.85, 0.05]) {
        ...
        if (canSee(candidate, groupPoint) && canSee(candidate, landmarkPoint)) {
  ...
  return {
    ...
    async run(stopRecords) {
      ...
      await rig.flyTo(
        guide.position.clone().add(new THREE.Vector3(9, 8, 12)),
        guide.position.clone().setY(1.4),
        2.8,
        Ease.cubicInOut
      );
      await wait(0.6);
