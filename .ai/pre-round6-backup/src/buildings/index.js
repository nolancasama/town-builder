...
const buildSize = (lot) => lot.buildSize || lot.size;

// These neighbouring reservations overlap at their outer corners. Their
// perimeter walks can merge harmlessly, but two full landmark models cannot
// occupy the paired pads at once. Keep that constraint in the existing lot
// selector so construction never creates a building-to-building collision.
const LOT_EXCLUSIONS = new Map([
  ['lot-school', new Set(['lot-west-big'])],
  ['lot-west-big', new Set(['lot-school', 'lot-north-yard'])],
  ['lot-north-yard', new Set(['lot-west-big'])],
  ['lot-stadium', new Set(['lot-southeast'])],
  ['lot-southeast', new Set(['lot-stadium'])],
  ['lot-northeast', new Set(['lot-northeast-big'])],
  ['lot-northeast-big', new Set(['lot-northeast'])],
]);

function lotIsFree(lot, takenLotIds) {
  if (takenLotIds.has(lot.id)) return false;
  const excluded = LOT_EXCLUSIONS.get(lot.id);
  return !excluded || ![...excluded].some((id) => takenLotIds.has(id));
}

function lotAllowsType(lot, type) {
  return (!lot.reservedFor || lot.reservedFor === type)
    && !lot.excludedTypes?.includes(type);
}
...
export function lotsFor(type, takenLotIds) {
  ...
  const minimum = def.minLotSize || [needed[0] * 0.62, needed[1] * 0.62];
  return LANDMARK_LOTS.filter(
    (lot) =>
      lotIsFree(lot, takenLotIds) &&
      lotAllowsType(lot, type) &&
      lot.zones.some((z) => def.zones.includes(z)) &&
      buildSize(lot)[0] >= minimum[0] &&
      buildSize(lot)[1] >= minimum[1]
  );
}
...
export function canPlace(type, takenLotIds) {
  const def = LANDMARKS[type];
  if (def.lot) {
    const preferred = LANDMARK_LOTS.find((lot) => lot.id === def.lot);
    if (preferred && lotIsFree(preferred, takenLotIds) && lotAllowsType(preferred, type)) return true;
  }
...
export function selectLot(type, takenLotIds) {
  ...
  if (def.lot) {
    ...
    if (preferred && lotIsFree(preferred, takenLotIds) && lotAllowsType(preferred, type)) return preferred;
  }
  const candidates = lotsFor(type, takenLotIds);
  if (!candidates.length) {
    return LANDMARK_LOTS.find(
      (lot) => lotIsFree(lot, takenLotIds) && lotAllowsType(lot, type)
    ) || null;
  }
  const want = area(def.footprint || [16, 14]);
  return candidates.sort((a, b) => Math.abs(area(buildSize(a)) - want) - Math.abs(area(buildSize(b)) - want))[0];
}
...
export async function loadLandmark(type, lot, rng) {
  ...
  // Builders continue receiving the original usable pad; `lot.size` now also
  // includes the dedicated perimeter sidewalk.
  const modelLot = { ...lot, size: buildSize(lot) };
  const args = { size: modelLot.size, sign: def.sign, rng, type, lot: modelLot };
  let model = null;
  ...
  if (def.factory) {
    ...
    if (glb) {
      model = fitToLot(glb, modelLot, def);
      source = 'glb';
    } else {
